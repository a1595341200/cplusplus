...
Engine::Engine(const std::string &name, std::shared_ptr<Looper> looper) : mName(name), mLooper(looper) {
	if (!mLooper) {
...
Engine::StateInfo *Engine::addStateIn(std::shared_ptr<IState> state, std::shared_ptr<IState> parent) {
...
bool Engine::isCustomMessage(int32_t id) {
	switch (id) {
		...
		case ENGINE_QUIT_CMD:return false;
	}
	return true;
}
