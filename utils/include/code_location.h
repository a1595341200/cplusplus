...
struct CodeLocation {
  ...
  */
  ...
  std::string ToString(Format format = Format::kFilename) const {
    ...
    out << (format == Format::kFilename ? FileNoPath() : file_and_path) << ":" << line_num << " "
        << function;
    return out.str();
  }
  // utf-8. Because on Windows we compile our code with "/utf-8". And we assume the other platforms
  // only use utf-8.
