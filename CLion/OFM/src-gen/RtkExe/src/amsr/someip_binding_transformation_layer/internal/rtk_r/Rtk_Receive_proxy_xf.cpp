 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  RtkExe/src/amsr/someip_binding_transformation_layer/internal/rtk_r/Rtk_Receive_proxy_xf.cpp
...
Rtk_ReceiveProxyXf::Rtk_ReceiveProxyXf(
...
void Rtk_ReceiveProxyXf::Start() noexcept { is_active_ = true; }

void Rtk_ReceiveProxyXf::Stop() noexcept { is_active_ = false; }
...
::ara::core::Result<void> Rtk_ReceiveProxyXf::RequestService() noexcept {
...
void Rtk_ReceiveProxyXf::ReleaseService() noexcept {
  std::lock_guard<std::mutex> const request_service_guard{request_service_mutex_};
...
bool Rtk_ReceiveProxyXf::SendMethodRequest(::vac::memory::MemoryBufferPtr<osabstraction::io::MutableIOBuffer> packet) {
  return client_manager_.SendMethodRequest(instance_id_, std::move(packet));
}

void Rtk_ReceiveProxyXf::HandleMethodResponse(
...
void Rtk_ReceiveProxyXf::OnServiceDown() {
}
