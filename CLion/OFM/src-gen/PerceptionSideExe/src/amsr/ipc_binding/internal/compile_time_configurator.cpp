 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  PerceptionSideExe/src/amsr/ipc_binding/internal/compile_time_configurator.cpp
...
void CompileTimeConfigurator<BindingCoreType>::RegisterInstanceSpecifierMappings(RuntimeInterface& runtime, BindingCoreType const& binding_core) noexcept {
  ...
  {
    // Map P-Port /JICA/SWC/PerceptionSideExe/PerceptionSide/PerceptionSideSIP to instance /JICA/Service/ProvidedIpcPerceptionSideSI_Out/ProvidedIpcPerceptionSideSI_Out
...
void CompileTimeConfigurator<BindingCoreType>::Cleanup(RuntimeInterface& runtime) const noexcept {
  ...
  {
    // Remove mapping P-Port /JICA/SWC/PerceptionSideExe/PerceptionSide/PerceptionSideSIP to instance /JICA/Service/ProvidedIpcPerceptionSideSI_Out/ProvidedIpcPerceptionSideSI_Out
    ::ara::core::InstanceSpecifier const instance_specifier{"PerceptionSideExe/PerceptionSide/PerceptionSideSIP"_sv};
    ::ara::com::InstanceIdentifier const instance_identifier{"Ipc:1097"_sv};
...
void CompileTimeConfigurator<BindingCoreType>::InitializeProvidedServiceDeployments(BindingCoreType& binding_core) noexcept {
  {
    // Instantiate provided service instances for service deployment '/JICA/Service/DeploymentIpc/IpcPerceptionSideSI_Out'.
    {
      ...
      sd_service_identifer.instance_id_ =  1097;
      sd_service_identifer.major_version_ =  1;
      sd_service_identifer.minor_version_  =  0;

      // Provided service instance '/JICA/Service/ProvidedIpcPerceptionSideSI_Out/ProvidedIpcPerceptionSideSI_Out'
      ...
      params.ipc_unicast_address_.ipc_unicast_domain_ = 35;
      params.ipc_unicast_address_.ipc_unicast_port_ = 35;

      binding_core.GetServerManager().EmplaceProvidedServiceInstance(sd_service_identifer, params, ::perceptionsi_out::PerceptionSideSI_Out::ServiceIdentifier);
    }
  }
}
