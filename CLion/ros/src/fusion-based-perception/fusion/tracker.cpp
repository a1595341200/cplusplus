        ...
        bool Tracker::Update(const LiDARObjectListPtr &lidar_obj_list,
            ...
            for (auto m : map) {
                if (m.second >= 0) {
                    ...
                    meas << lidar_obj->x, lidar_obj->y, lidar_obj->velo_x, lidar_obj->velo_y,
                        lidar_obj->label;
                    ...
                    try {
                        ...
                        global_obj_list_->objs[m.second]->life_duration =
                            (dura < startup_duration_ ? dura + 1 : startup_duration_);
                        std::cout << "updating exist global object: "
                                  << global_obj_list_->objs[m.second]->ToString() << std::endl;
                    ...
                    new_obj->y = lidar_obj->y;
                    new_obj->z = lidar_obj->z;
        ...
        bool Tracker::Update(const RadarObjectListPtr &radar_obj_list,
            ...
            for (auto m : map) {
                if (m.second >= 0) {
                    ...
                    Eigen::VectorXd meas(5);
                    meas << radar_obj->x, radar_obj->y, radar_obj->velo_x, radar_obj->velo_y, 0;
                    radar_mea.meas = meas;
        ...
        bool Tracker::Update(const CameraObjectListPtr &camera_obj_list,
            ...
            for (auto m : map) {
                if (m.second >= 0) {
                    ...
                    Eigen::VectorXd meas(5);
                    meas << cam_obj->ux, cam_obj->vy, cam_obj->width, cam_obj->height,
                        float(cam_obj->label);
                    cam_mea.meas = meas;
                    ...
                    State state;
                    state.time_ns = global_obj->time_ns;
                    meas << global_obj->ux, global_obj->vy, global_obj->width_2d,
                        global_obj->height_2d, float(global_obj->label);
                    state.meas = meas;
