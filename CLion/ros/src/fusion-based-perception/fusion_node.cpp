...
inline void printMregeRes(
    const std::vector<std::tuple<double, proto_input::FileTag, std::string> > &data) {
    for (auto &v : data)
...
int main(int argc, char **argv) {
    ...
    std::vector<std::tuple<double, proto_input::FileTag, std::string> > &&data_frame_seq =
        proto_input::mergeInputSequence({
            proto_input::buildSensorInputSequence(argv[2], "Location",
                                                  proto_input::FileTag::LOCATION),
    ...
    for (auto it = data_frame_seq.begin(); it != data_frame_seq.end();) {
        ...
        std::shared_ptr<kit::perception::fusion::Frame> frame =
            std::make_shared<kit::perception::fusion::Frame>();
        //get camera detection results
        if (std::get<1>(*it) == proto_input::FileTag::CAMERA) {
            proto_input::CameraObject raw_co;
            kit::perception::fusion::CameraObjectListPtr co_list =
                std::make_shared<kit::perception::fusion::CameraObjectList>();
            itp.loadFile(std::get<2>(*it));
            while (itp.lex.getToken() == ezcfg::Token::ID &&
                   itp.lex.getTokenText() == "perception_obstacle") {
                itp.lex.next();
                itp.parse(raw_co);
        ...
        if (std::get<1>(*it) == proto_input::FileTag::LIDAR) {
            ...
            kit::perception::fusion::LiDARObjectListPtr lo_list =
                std::make_shared<kit::perception::fusion::LiDARObjectList>();
            itp.loadFile(std::get<2>(*it));
            while (itp.lex.getToken() == ezcfg::Token::ID &&
                   itp.lex.getTokenText() == "perception_obstacle") {
                itp.lex.next();
        ...
        kit::perception::fusion::FusionObjectListPtr fusion_res =
            std::make_shared<kit::perception::fusion::FusionObjectList>(
                kit::perception::fusion::FusionObjectList());
