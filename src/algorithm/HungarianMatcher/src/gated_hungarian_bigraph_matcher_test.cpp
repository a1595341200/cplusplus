...
namespace apollo::perception::common {
...
class GatedHungarianMatcherTest : public testing::Test {
 ...
 protected:
  void SetUp() {
    optimizer_ = new GatedHungarianMatcher<float>(1000);
  }
  void TearDown() {
    delete optimizer_;
    optimizer_ = nullptr;
...
TEST_F(GatedHungarianMatcherTest, test_Match_Minimize_badcase1) {
  ...
  for (auto& val : assignments) {
    std::cout << "assignments: " << val.first<< " : " << val.second << std::endl;
  }

  for (auto& val : unassigned_rows) {
    std::cout << "unassigned_rows: " << val << std::endl;
  }

  for (auto& val : unassigned_cols) {
    std::cout << "unassigned_cols: " << val << std::endl;
  }
