 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  PerceptionExe/include/FreeSpaceIDT/impl_type_freespaceidt.h
 *        \brief
 *
 *      \details
 *
...
#ifndef PERCEPTIONEXE_INCLUDE_FREESPACEIDT_IMPL_TYPE_FREESPACEIDT_H_
#define PERCEPTIONEXE_INCLUDE_FREESPACEIDT_IMPL_TYPE_FREESPACEIDT_H_
...
#include "FsBorderTypeIDT/impl_type_fsbordertypeidt.h"
...
/*!
 * \brief Type FreeSpaceIDT.
 * \remark generated
 *
 * IMPLEMENTATION-DATA-TYPE /CppImplementationDataTypes/FreeSpaceIDT
 */
struct FreeSpaceIDT {
  using BorderDst_generated_type = float32IDT::float32IDT;
  using BorderType_generated_type = FsBorderTypeIDT::FsBorderTypeIDT;
  using Confidence_generated_type = float32IDT::float32IDT;
  using HeightFreeSpace_generated_type = float32IDT::float32IDT;
  ...
  BorderDst_generated_type BorderDst;
  BorderType_generated_type BorderType;
  Confidence_generated_type Confidence;
  HeightFreeSpace_generated_type HeightFreeSpace;
...
/*!
 * \brief   Compare for equality with another FreeSpaceIDT instance.
 ...
 *          use your own.
 */
inline bool operator==(FreeSpaceIDT const& l,
                       FreeSpaceIDT const& r) noexcept {
  ...
  return (&l == &r) || (compare_float(l.BorderDst, r.BorderDst)
                         && (l.BorderType == r.BorderType)
                         && compare_float(l.Confidence, r.Confidence)
                         && compare_float(l.HeightFreeSpace, r.HeightFreeSpace)
...
/*!
 * \brief   Compare for inequality with another FreeSpaceIDT instance.
 * \details The method for comparing floating point members is to check whether they are at most
 *          one machine epsilon apart. This method works best for comparing numbers between
 *          1.0 and 2.0. Larger numbers will only be equal if they match exactly, and smaller
 *          numbers that are relatively far apart will be considered equal.
...
#endif  // PERCEPTIONEXE_INCLUDE_FREESPACEIDT_IMPL_TYPE_FREESPACEIDT_H_
