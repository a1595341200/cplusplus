...
const std::string c_listen_addr = "192.168.188.128";
...
class UDPMulticastSocket {
 public:
  UDPMulticastSocket(boost::asio::io_service &io_service)
      : socket_(io_service),
        timer_(io_service),
        endpoint_(boost::asio::ip::address_v4::from_string(c_multicast_addr), c_multicast_port) {
 ...
 private:
  void do_receive() {
    socket_.async_receive_from(boost::asio::buffer(buff_), sender_endpoint,
                               [this](const boost::system::error_code ec, std::size_t length) {
                                 if (!ec) {
                                   std::cout << "receive:";
                                   std::cout.write(buff_.data(), length);
                                   std::cout << std::endl;
                                   std::cout << sender_endpoint.address().to_string() << ":"
  ...
  void do_send() {
    ...
    socket_.async_send_to(boost::asio::buffer(msg), endpoint_,
                          [this](const boost::system::error_code &ec, std::size_t /*length*/) {
