...
void multi_client_performance(size_t n) {
  ...
  for (int i = 0; i < n; ++i) {
    group.emplace_back(
        std::make_shared<std::thread>(test_performance1));  // []{test_performance1();});
  }
  for (auto &p : group) {
    p->join();
  }
...
void wait_for_notification(rpc_client &client) {
  client.async_call<0>("sub", [&client](const asio::error_code &ec, string_view data) {
    auto str = as<std::string>(data);
    std::cout << str << '\n';
...
void test_ssl() {
  bool is_ssl = true;
  rpc_client client;
  client.set_error_callback([](asio::error_code ec) { std::cout << ec.message() << "\n"; });
