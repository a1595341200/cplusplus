 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  EHRExe/src/amsr/ipc_binding_transformation_layer/internal/life_cycle_manager_xf.cpp
...
void LifeCycleManagerXf::InitializeServiceDiscoverySkeletonXfs(BindingCoreType& binding_core) noexcept{
  ...
  // Service Config for Service Interface '/JICA/Service/Interface/EHRSI_Out'
  {
    ...
    logger_.LogDebug(
        [](::ara::log::LogStream& s) {
          // VCA_IPCB_LOGSTREAM_CONCATENATION_IS_SILENT
          s << "Initializing service skeleton '/JICA/Service/Interface/EHRSI_Out'.";
        },
        Logger::LogLocation{__func__, __LINE__});
    ...
    for (ServiceConfig::ProvidedServiceInstanceConfigRef const provided_service_instance :
      ...
      sd_skeleton_xfs_.emplace_back(std::make_unique<SdSkeletonXfType>(
          ...
          ::ehrsi_out::skeleton::EHRSI_OutSkeleton::GetServiceDiscovery(),
          service_config));
    }
  }
}
