 ...
 *  FILE DESCRIPTION
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  EDRExe/src/amsr/ipc_binding_transformation_layer/internal/life_cycle_manager_xf.cpp
...
void LifeCycleManagerXf::InitializeServiceDiscoverySkeletonXfs(BindingCoreType& binding_core) noexcept{
  ...
  // Service Config for Service Interface '/JICA/Service/Interface/EDRSI_Out'
  {
    ...
    logger_.LogDebug(
        [](::ara::log::LogStream& s) {
          // VCA_IPCB_LOGSTREAM_CONCATENATION_IS_SILENT
          s << "Initializing service skeleton '/JICA/Service/Interface/EDRSI_Out'.";
        },
        Logger::LogLocation{__func__, __LINE__});
    using SdSkeletonXfType = ::amsr::ipc_binding_transformation_layer::internal::ServiceDiscoverySkeletonXf<
        ::amsr::ipc_binding_transformation_layer::internal::edrsi_out::EDRSI_OutProvidedIpcServiceInstance,
    ...
    for (ServiceConfig::ProvidedServiceInstanceConfigRef const provided_service_instance :
      ...
      sd_skeleton_xfs_.emplace_back(std::make_unique<SdSkeletonXfType>(
          ...
          ::edrsi_out::skeleton::EDRSI_OutSkeleton::GetServiceDiscovery(),
          service_config));
    }
  }
}
