...
class SerializerOFM_St_SOCFusedSts {
 public:
  ...
  /*!
   * \brief Returns the maximum buffer size for the data prototype service event /JICA/Service/Interface/ObjectFusionModelSI_Out/OFM_St_SOCFusedSts.
   *
   * \return      Calculated buffer size for serialization.
   * \pre         -
   * \context     App
  ...
  static void Serialize(serialization::Writer &writer, uint16_t const &data) noexcept;
