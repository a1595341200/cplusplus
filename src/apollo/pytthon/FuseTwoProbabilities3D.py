...
def test() :
    ...
    for i in range(len(x)) :
            z.append(FuseTwoProbabilities(x[i],y[i]))
    # print(z)
    # 线
    fig = plt.figure()
    ax = fig.add_subplot(projection='3d')
    ...
    # c颜色，marker：样式*雪花
    ax.scatter(xs=x, ys=y, zs=z, c="g")
    # ax.scatter(xs=x, ys=y, c="b")
    # ax.scatter(ys=np.linspace(0,0,100),xs=x, zs=z, c="y")
    # ax.scatter(xs =np.linspace(0,0,100),ys=y, zs=z, c="r")
    ax.set_zlabel('z')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.show()
...
def test1() :
    ...
    for i in range(len(x)) :
        for j in range(len(y)) :
            ...
            my.append(y[j])
            empty.append(0)
            z.append(FuseTwoProbabilities(x[i],y[j]))
    # print(z)
    # 线
    fig = plt.figure()
    ax = fig.add_subplot(projection='3d')
    ...
    # c颜色，marker：样式*雪花
    ax.scatter(xs=mx, ys=my, zs=z, c="g")
    ax.set_zlabel('z')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.show()
...
def test2() :
    ...
    norm1 = stats.norm(loc=0,scale=0.5)
    ...
    plt.plot(x,y,color='orangered',label = r'$\sigma$ = 1')
    plt.plot(x,y1,color='g',label = r'$\sigma$ = 0.5')
    plt.legend()
    ax=plt.gca()
