...
static const std::vector<uint64_t> fod_subsets = {F111, FA18, P3C, FAST, UNKNOWN};
static const std::map<uint64_t, std::string> hypo_names = {
    {F111, "F111"}, {FA18, "FA18"}, {P3C, "P3C"}, {FAST, "FAST"}, {UNKNOWN, "UNKNOWN"}};

class DSTEvidenceTest : public ::testing::Test {
 public:
  DSTEvidenceTest() : sensor1_dst_("test"), sensor2_dst_("test"), fused_dst_("test") {
    DstManager *dst_manager = DstManager::Instance();
    std::vector<std::string> fod_subset_names = {"F111", "FA18", "P3C", "FAST", "UNKNOWN"};
    dst_manager->AddApp("test", fod_subsets, fod_subset_names);
    vec_equal_ = [](const std::vector<double> &vec, const std::vector<double> &gt) {
 ...
 protected:
  void run(const std::vector<double> &sensor1_data, const std::vector<double> &sensor2_data) {
  ...
  void assign_dst_test() {
    std::map<uint64_t, double> dst_map = {{F111, 0.3}, {FA18, 0.18}, {FAST, 0.42}, {UNKNOWN, 0.1}};
  ...
  void add_dst_test1() {
    ...
    std::vector<double> fused_dst_gt = {0.546233074876, 0.160861011457, 0.00405045712302,
                                        0.285383636153, 0.00347182039116};
    std::vector<double> fused_spt_gt = {0.5462330748755931, 0.1608610114570073,
                                        0.004050457123018168, 0.9924777224858234, 1.0};
    std::vector<double> fused_pls_gt = {0.8350885314199745, 0.4497164680013887,
                                        0.007522277514176598, 0.9959495428769819, 1.0};
    std::vector<double> fused_prob_gt = {0.69008217, 0.30471010, 0.00520773, 0.99479227, 1.0000000};
  ...
  void add_dst_test2() {
    ...
    run(sensor1_data, sensor2_data);
    std::vector<double> fused_dst_gt = {0.6318805610013573, 0.30990800784195444,
                                        0.034685567787664004, 0.019001658874981148,
                                        0.004524204494043131};
    std::vector<double> fused_spt_gt = {0.6318805610013573, 0.30990800784195444,
                                        0.034685567787664004, 0.9607902277182928, 1.0};
    std::vector<double> fused_pls_gt = {0.6554064243703815, 0.3334338712109787,
                                        0.039209772281707134, 0.9653144322123359, 1.0};
    std::vector<double> fused_prob_gt = {0.64288946, 0.32091691, 0.03619364, 0.96380636,
                                         1.00000000};
    std::vector<double> fused_uct_gt(5);
    for (size_t i = 0; i < fused_spt_gt.size(); ++i) {
      fused_uct_gt[i] = fused_pls_gt[i] - fused_spt_gt[i];
  ...
  void multipy_dst_test() {
    std::vector<double> dst_data = {0.13891389, 0.57305731, 0.02580258, 0.22472247, 0.03750375};
    std::vector<double> gt = {0.11113111, 0.45844584, 0.02064206, 0.17977798, 0.230003};
    Dst dst("test");
 ...
 private:
  std::function<void(const std::vector<double> &vec, const std::vector<double> &gt)> vec_equal_;
  // InitApp _init_app;
  Dst sensor1_dst_;
  Dst sensor2_dst_;
  Dst fused_dst_;
...
TEST_F(DSTEvidenceTest, assign_dst_test) {
  DSTEvidenceTest::assign_dst_test();
}
TEST_F(DSTEvidenceTest, add_dst_test1) {
  DSTEvidenceTest::add_dst_test1();
}
TEST_F(DSTEvidenceTest, add_dst_test2) {
  DSTEvidenceTest::add_dst_test2();
}
