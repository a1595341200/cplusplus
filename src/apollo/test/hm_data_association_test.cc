...
TEST(ProjectionCacheFrame, test_projection_cache_query_object_1) {
  ...
  ProjectionCacheObject* cache_object_ptr_2 = projection_cache_frame.BuildObject(2);
...
TEST(ProjectionCache, test_projection_cache_query_object_2) {
  ...
  ProjectionCacheObject* cache_object_ptr =
      projection_cache.BuildObject("lidar", 1.0, "camera_obstacle", 1.0, 1);
  EXPECT_FALSE(projection_cache.QueryObject("lidar", 2.0, "camera_obstacle", 1.0, 1) != nullptr);
  EXPECT_FALSE(projection_cache.QueryObject("lidar", 1.0, "camera_obstacle", 2.0, 1) != nullptr);
  EXPECT_FALSE(projection_cache.QueryObject("lidar", 1.0, "camera_obstacle", 1.0, 2) != nullptr);
  EXPECT_TRUE(projection_cache.QueryObject("lidar", 1.0, "camera_obstacle", 1.0, 1) != nullptr);
...
TEST(ComputePtsBoxLocationSimilarity, test_compute_pts_box_location_sim) {
  ...
  EXPECT_EQ(ComputePtsBoxLocationSimilarity(cache_ptr, cache_object_ptr, camera_bbox), 1e-6);
  ...
  EXPECT_GT(ComputePtsBoxLocationSimilarity(cache_ptr, cache_object_ptr, camera_bbox), 1 - 1e-3);
  delete cache_ptr;
  cache_ptr = nullptr;
...
TEST(ComputePtsBoxShapeSimilarity, test_compute_pts_box_shape_sim) {
  ...
  base::BBox2DF camera_bbox = base::BBox2DF(100, 100, 200, 200);
  ProjectionCacheObject* cache_object_ptr = new ProjectionCacheObject;
  EXPECT_EQ(ComputePtsBoxShapeSimilarity(cache_ptr, cache_object_ptr, camera_bbox), 1e-3);
  cache_object_ptr->SetStartInd(0);
  cache_object_ptr->SetEndInd(2);
  ...
  EXPECT_GE(ComputePtsBoxShapeSimilarity(cache_ptr, cache_object_ptr, camera_bbox), 1 - 1e-3);
  delete cache_ptr;
  cache_ptr = nullptr;
  delete cache_object_ptr;
  cache_object_ptr = nullptr;
...
TEST(ComputePtsBoxSimilarity, test_compute_pts_box_sim) {
  ...
  cache_object_ptr->SetEndInd(2);
  cache_object_ptr->SetBox(base::BBox2DF(100, 100, 200, 200));
  EXPECT_GT(ComputePtsBoxSimilarity(cache_ptr, cache_object_ptr, camera_bbox), 1 - 1e-3);
  delete cache_ptr;
  cache_ptr = nullptr;
  delete cache_object_ptr;
  cache_object_ptr = nullptr;
...
TEST(ComputeRadarCameraXSimilarity, test_compute_radar_camera_x_sim) {
  ...
  EXPECT_GT(ComputeRadarCameraXSimilarity(velo_ct_x, camera_ct_x, size_x, x_similarity_param),
...
TEST(ComputeRadarCameraYSimilarity, test_compute_radar_camera_y_sim) {
  ...
  EXPECT_GT(ComputeRadarCameraYSimilarity(velo_ct_y, camera_ct_y, size_y, y_similarity_param),
...
TEST(ComputeRadarCameraHSimilarity, test_compute_radar_camera_h_sim) {
  ...
  double size_y = 100;
  base::ObjectPtr camera_object_ptr = std::shared_ptr<base::Object>(new base::Object);
  ...
  EXPECT_GT(ComputeRadarCameraHSimilarity(sensor_radar_ptr, camera_const_sensor_ptr, size_y,
                                          radar_box2d_vertices, h_similiar_params),
            h_similiar_params.scale_positive_max_p_ - 1e-3);
}
...
TEST(ComputeRadarCameraWSimilarity, test_compute_radar_camera_w_sim) {
  ...
  EXPECT_GT(ComputeRadarCameraWSimilarity(radar, width, size_x, radar_box2d_vertices,
                                          w_similarity_params),
...
TEST(ComputeRadarCameraLocSimilarity, test_compute_radar_camera_loc_sim) {
  ...
  dbg(camera_object_ptr->ToString());
  base::ObjectConstPtr camera_const_object_ptr(camera_object_ptr);
  const SensorFramePtr camera_frame_ptr = nullptr;
  SensorObjectConstPtr camera_const_sensor_ptr(
      new SensorObject(camera_const_object_ptr, camera_frame_ptr));
  ...
  EXPECT_GT(ComputeRadarCameraLocSimilarity(radar_ct, camera_const_sensor_ptr, world2camera_pose,
                                            loc_similiarity_params),
...
TEST(ComputeRadarCameraVelocitySimilarity, test_compute_radar_camera_vel_sim) {
  ...
  base::ObjectPtr radar_object_ptr = std::shared_ptr<base::Object>(new base::Object);
  ...
  EXPECT_LT(ComputeRadarCameraVelocitySimilarity(radar_const_sensor_ptr, camera_const_sensor_ptr),
            0.8);
}
