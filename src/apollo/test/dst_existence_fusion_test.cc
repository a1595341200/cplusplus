...
TEST(DstExistFusionTest, 1) {
  ToicDstMaps t;
  dbg(std::bitset<sizeof(uint64_t)>(t.NTOIC));
  dbg(std::bitset<sizeof(uint64_t)>(t.TOIC));
  dbg(std::bitset<sizeof(uint64_t)>(t.TOICUNKNOWN));
  ExistenceDstMaps e;
  dbg(std::bitset<sizeof(uint64_t)>(e.EXIST));
  dbg(std::bitset<sizeof(uint64_t)>(e.NEXIST));
  dbg(std::bitset<sizeof(uint64_t)>(e.EXISTUNKNOWN));
}
}  // namespace fusion
}  // namespace perception
}  // namespace apollo
