...
DstTypeFusion::DstTypeFusion(TrackPtr track) : BaseTypeFusion(track), fused_dst_(name_) {
  ...
  fused_dst_ = fused_dst_ + sensor_dst * GetReliability(sensor_obj->GetSensorId());
  UpdateTypeState();
}
...
bool DstTypeFusion::Init() {
  ...
  //         << " reliability for unknown: " << options_.sensor_reliability_for_unknown_[camera_id];
  // }
  ...
  //         << " reliability for unknown: " << options_.sensor_reliability_for_unknown_[lidar_id];
  // }
...
void DstTypeFusion::UpdateWithMeasurement(const SensorObjectPtr measurement,
  ...
  ADEBUG << "type_probs: " << vector2string<float>(measurement->GetBaseObject()->type_probs);
  fused_dst_ = fused_dst_ + measurement_dst * GetReliability(measurement->GetSensorId());
...
void DstTypeFusion::UpdateWithoutMeasurement(const std::string &sensor_id,
                                             double measurement_timestamp, double target_timestamp,
  ...
  if (sensor_manager->IsCamera(sensor_id)) {
    ...
    base::BaseCameraModelPtr camera_model = sensor_data_manager->GetCameraIntrinsic(sensor_id);
    ACHECK(camera_model != nullptr) << "Failed to get camera intrinsic for " << sensor_id;

    Eigen::Affine3d sensor2world_pose;
    bool status =
        sensor_data_manager->GetPose(sensor_id, measurement_timestamp, &sensor2world_pose);
    ...
    std::map<uint64_t, double> fp_dst_map = {{DstMaps::OTHERS, 0.7}, {DstMaps::UNKNOWN, 0.3}};
    Dst fp_dst(name_);
    fp_dst.SetBba(fp_dst_map);
    fused_dst_ = fused_dst_ + (fp_dst * in_view_ratio * occlusion_score) *
...
double DstTypeFusion::GetReliability(const std::string &sensor_id) const {
  ...
  if (find_res == options_.sensor_reliability_.end()) {
    ADEBUG << "the sensor type: " << sensor_id << " is not supported by class fusion";
    return 0.0;
  }
...
double DstTypeFusion::GetReliabilityForUnKnown(const std::string &sensor_id,
  ...
  double prob = (common::SensorManager::Instance()->IsCamera(sensor_id) && is_night) ? 0.1 : 1.0;
...
void DstTypeFusion::UpdateTypeState() {
  ...
  track_ref_->GetFusedObject()->GetBaseObject()->type = static_cast<base::ObjectType>(object_type);
  // modify subtype
  auto type = base::kSubType2TypeMap.find(track_ref_->GetFusedObject()->GetBaseObject()->sub_type);
  if (type != base::kSubType2TypeMap.end() &&
      type->second != track_ref_->GetFusedObject()->GetBaseObject()->type) {
    track_ref_->GetFusedObject()->GetBaseObject()->sub_type = base::ObjectSubType::UNKNOWN;
  }

  // update type_probs
  std::vector<float> type_probs(static_cast<int>(base::ObjectType::MAX_OBJECT_TYPE), 0);
