...
double ComputePtsBoxLocationSimilarity(const ProjectionCachePtr& cache,
  ...
  if (end_ind - start_ind >= check_augmented_iou_minimum_pts_num) {
    ...
    float augmented_iou = CalculateAugmentedIOUBBox(velo_bbox, camera_bbox, augmented_buffer);
  ...
  double location_similarity = 1 - ChiSquaredCdf2TableFun(square_norm_mean_pixel_dist);
...
double ComputePtsBoxShapeSimilarity(const ProjectionCachePtr& cache,
  ...
  Eigen::Vector2d box_size_diff = (velo_box_size - camera_box_size).array() / mean_box_size.array();
  // assuming the normalized distance obeys gauss distribution
  double square_norm_box_size_diff = box_size_diff.x() * box_size_diff.x() / x_std_dev / x_std_dev +
                                     box_size_diff.y() * box_size_diff.y() / y_std_dev / y_std_dev;
  ADEBUG << "camera_box_size@(" << camera_box_size(0) << "," << camera_box_size(1) << "); "
  ...
  // use chi-square distribution. Cauchy may be more reasonable.
  double shape_similarity = 1 - ChiSquaredCdf2TableFun(square_norm_box_size_diff);
  // for numerical stability
...
double ComputePtsBoxSimilarity(const ProjectionCachePtr& cache, const ProjectionCacheObject* object,
                               const base::BBox2DF& camera_bbox) {
  double location_similarity = ComputePtsBoxLocationSimilarity(cache, object, camera_bbox);
  double shape_similarity = ComputePtsBoxShapeSimilarity(cache, object, camera_bbox);
  double fused_similarity = FuseTwoProbabilities(location_similarity, shape_similarity);
...
double ComputeRadarCameraXSimilarity(const double velo_ct_x, const double camera_ct_x,
                                     const double size_x, const XSimilarityParams& params) {
  double x_diff = std::abs(velo_ct_x - camera_ct_x) / size_x;
  dbg(x_diff);
  double x_similarity =
      WelshVarLossFun(x_diff, params.welsh_loss_thresh_, params.welsh_loss_scale_);

  dbg(x_similarity);
  x_similarity = ScalePositiveProbability(x_similarity, params.scale_positive_max_p_,
                                          params.scale_positive_th_p_);
  dbg(x_similarity);

  return x_similarity;
}
double ComputeRadarCameraYSimilarity(const double velo_ct_y, const double camera_ct_y,
                                     const double size_y, const YSimilarityParams& params) {
  // double y_diff =
  //     std::abs(velo_ct_y - camera_ct_y + size_y * params.smooth_factor_) /
  //     size_y;

  double y_diff =
      std::max(std::abs(velo_ct_y - camera_ct_y) - size_y * params.smooth_factor_, 0.0) / size_y;
  dbg(y_diff);

  double normalized_y_diff = y_diff * y_diff / params.diff_std_dev_ / params.diff_std_dev_;
  dbg(normalized_y_diff);
  double y_similarity = 1 - ChiSquaredCdf1TableFun(normalized_y_diff);
  dbg(y_similarity);

  y_similarity = BoundedScalePositiveProbability(y_similarity, params.bounded_scale_positive_max_p_,
                                                 params.bounded_scale_positive_min_p_);
  dbg(y_similarity);

  return y_similarity;
}
double ComputeRadarCameraHSimilarity(const SensorObjectConstPtr& radar,
                                     const SensorObjectConstPtr& camera, const double size_y,
  ...
  if (camera_height > kFloatEpsilon) {
    ...
    for (size_t i = 0; i < 4; ++i) {
      double img_car_height =
          std::abs(radar_box2d_vertices[i + 4].y() - radar_box2d_vertices[i].y());
      min_height_diff = std::min(std::abs(img_car_height - size_y), min_height_diff);
    }
    min_height_diff /= size_y;
    double normalized_min_height_diff =
        min_height_diff * min_height_diff / params.diff_std_dev_ / params.diff_std_dev_;
    height_similarity = 1 - ChiSquaredCdf1TableFun(normalized_min_height_diff);
    height_similarity = ScalePositiveProbability(height_similarity, params.scale_positive_max_p_,
                                                 params.scale_positive_th_p_);
  }
  return height_similarity;
}
double ComputeRadarCameraWSimilarity(const SensorObjectConstPtr& radar, const double width,
                                     const double size_x,
                                     const std::vector<Eigen::Vector2d>& radar_box2d_vertices,
                                     const WSimilarityParams& params) {
  std::vector<double> radar_box2d_xs = {radar_box2d_vertices[0].x(), radar_box2d_vertices[1].x(),
                                        radar_box2d_vertices[2].x(), radar_box2d_vertices[3].x()};
  for (double& x : radar_box2d_xs) {
    x = Bound(x, width, 0.0);
  }
  auto min_max_xs = std::minmax_element(radar_box2d_xs.begin(), radar_box2d_xs.end());
  ...
  width_similarity = BoundedScalePositiveProbability(
      width_similarity, params.bounded_scale_positive_max_p_, params.bounded_scale_positive_min_p_);
...
double ComputeRadarCameraLocSimilarity(const Eigen::Vector3d& radar_ct,
  ...
  dbg(ct_diff);
  double ct_similarity =
      WelshVarLossFun(ct_diff, params.welsh_loss_thresh_, params.welsh_loss_scale_);
  dbg(params.welsh_loss_thresh_);

  dbg(ct_similarity);

  ct_similarity = ScalePositiveProbability(ct_similarity, params.scale_positive_max_p_,
                                           params.scale_positive_th_p_);
  dbg(params.scale_positive_max_p_);
...
double ComputeRadarCameraVelocitySimilarity(const SensorObjectConstPtr& radar,
  ...
  dbg(scalar_radar_velocity);
  float scalar_camera_velocity = camera_velocity.norm();
  dbg(scalar_camera_velocity);
  ...
  if (std::max(scalar_radar_velocity, scalar_camera_velocity) > 2) {
    ...
    dbg(diff_velocity_ratio);
    const float velocity_std = 0.15f;
    // const float velocity_std = 0.2f;
    const float max_velocity_p = 0.9f;
    const float th_velocity_p = 0.5f;
    dbg(diff_velocity_ratio * diff_velocity_ratio / velocity_std / velocity_std);
    float velocity_score =
        static_cast<float>(1 - ChiSquaredCdf1TableFun(diff_velocity_ratio * diff_velocity_ratio /
                                                      velocity_std / velocity_std));
    dbg(velocity_score);

    velocity_score =
        static_cast<float>(ScalePositiveProbability(velocity_score, max_velocity_p, th_velocity_p));
    dbg(velocity_score);
