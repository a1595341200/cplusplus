...
#include <Log.h>
#define TAG "GlobalTracker"
using namespace JPDAFTracker;

GlobalTracker::GlobalTracker(const TrackerParam& _param) : Tracker(_param) {
  trackID_ = 0;
  init_ = true;
...
void GlobalTracker::track(const GlobalTracker::Detections& _detections) {
  if (init_) {
    ...
    if (q.total() > 0) {
      ...
      SLOG(INFO) << "association_matrices start";
      debugVector("association_matrices", association_matrices);
  ...
  if (prevDetSize == 0) {
    ...
    for (uint i = 0; i < prevDetSize; ++i) {
      for (uint j = 0; j < deteSize; ++j) {
        costs.at(i + j * prevDetSize) =
            euclideanDist(not_associated_.at(j), prev_detections_.at(i));
    ...
    for (uint i = 0; i < rows; ++i) {
      for (uint j = 0; j < cols; ++j) {
        if (assigmentsBin.at<int>(i, j)) {
          ...
          std::shared_ptr<Track> tr(new Track(param_.dt, param_.target_delta,
                                              not_associated_.at(j).x(), not_associated_.at(j).y(),
