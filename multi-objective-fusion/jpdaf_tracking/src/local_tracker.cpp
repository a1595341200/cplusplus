...
LocalTracker::LocalTracker(const TrackerParam& _param) : Tracker(_param) {
  time = cv::getTickCount();
}

void LocalTracker::track(const std::vector<Detection>& _detections, VecBool& _isAssoc,
                         uint& _trackID) {
  uint j = 0;
  for (auto& track : tracks_) {
    track->predict();
    if (track->getId() == -1 && track->isAlive() &&
        track->getEntropy() == Track::TrackState::ACCEPT) {
