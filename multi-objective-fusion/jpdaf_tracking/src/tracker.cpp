...
void Tracker::drawTracks(cv::Mat& _img) const {
  ...
  for (const auto& track : tracks_) {
    if (track->getId() != -1) {
      ...
      cv::putText(_img, ss.str(), p, cv::FONT_HERSHEY_SIMPLEX, 0.50, cv::Scalar(0, 255, 0), 2,
                  cv::LINE_AA);
    }
  }
}
  ...
  for (uint i = 0; i < hyp_num; ++i) {
    ...
    if (param_.pd == 1) {
      ...
      for (uint j = 0; j < tracksize; ++j) {
        ...
        prior = prior * std::pow(param_.pd, target_indicator) *
                std::pow((1 - param_.pd), (1 - target_indicator));
      }
    }
...
Tracker::Matrices Tracker::generate_hypothesis(const Vec2f& _selected_detections,
                                               const cv::Mat& _q) {
  Utils::Timer timer;
  debugVector("_selected_detections", _selected_detections);
  SLOG(INFO) << "q \n" << _q;

  uint validationIdx = _q.rows;
  // All the measurements can be generated by the clutter track
  Eigen::MatrixXf A_Matrix = Eigen::MatrixXf::Zero(_q.rows, _q.cols);
  A_Matrix.col(0).setOnes();
  Matrices tmp_association_matrices(MAX_ASSOC, A_Matrix);
  // SLOG(INFO) << "tmp_association_matrices sizeof : " << sizeof(tmp_association_matrices);
  uint hyp_num = 0;
    ...
    for (uint i = 0; i < _q.rows; ++i) {
      for (uint j = 1; j < _q.cols; ++j) {
        if (_q.at<int>(i, j))  // == 1
        {
          ...
          // SLOG(INFO) << "tmp_association_matrices \n"
          //            << hyp_num << " : " << tmp_association_matrices.at(hyp_num);
          ++hyp_num;
          if (j == _q.cols - 1)
            continue;
          for (uint l = 0; l < _q.rows; ++l) {
            if (l != i) {
              ...
              {
                if (_q.at<int>(l, m)) {
                  // SLOG(INFO) << "second tmp_association_matrices\n"
                  //            << hyp_num << " : " << tmp_association_matrices.at(hyp_num);
                  tmp_association_matrices.at(hyp_num)(i, 0) = 0;
                  // SLOG(INFO) << "tmp_association_matrices i, 0\n"
                  //            << hyp_num << " : " << i << ",0  val = " << 0;
                  tmp_association_matrices.at(hyp_num)(i, j) = 1;
                  // SLOG(INFO) << "tmp_association_matrices i,j\n"
                  //            << hyp_num << " " << i << "," << j << "val 1";
                  tmp_association_matrices.at(hyp_num)(l, 0) = 0;

                  // SLOG(INFO) << "tmp_association_matrices l,0\n"
                  //            << hyp_num << " " << l << ", 0 "
                  //            << "val 0";
                  tmp_association_matrices.at(hyp_num)(l, m) = 1;

                  // SLOG(INFO) << "tmp_association_matrices l,m\n"
                  //            << hyp_num << " " << l << "," << m << "val 1";
  ...
  SLOG(INFO) << "association_matrices size " << association_matrices.size();
