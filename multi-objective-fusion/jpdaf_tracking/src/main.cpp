...
cv::Mat H = (cv::Mat_<double>(3, 3) << -2.24449, -8.3239, 1146.4, -0.913708, -10.0553, 1058.36,
...
void calcInvProjection(const cv::Point2f& point, double& x, double& y) {
  x = (point.x * H_inv.at<double>(0, 0) + point.y * H_inv.at<double>(0, 1) +
       H_inv.at<double>(0, 2)) /
      (point.x * H_inv.at<double>(2, 0) + point.y * H_inv.at<double>(2, 1) +
       H_inv.at<double>(2, 2));
  y = (point.x * H_inv.at<double>(1, 0) + point.y * H_inv.at<double>(1, 1) +
       H_inv.at<double>(1, 2)) /
      (point.x * H_inv.at<double>(2, 0) + point.y * H_inv.at<double>(2, 1) +
       H_inv.at<double>(2, 2));
}
...
int main(int argc, char** argv) {
  ...
  std::shared_ptr<JPDAFTracker::Tracker> tracker =
      std::make_shared<JPDAFTracker::GlobalTracker>(params);
  ...
  // Setup output video
  cv::VideoWriter output_cap("output.avi", cv::VideoWriter::fourcc('D', 'I', 'V', 'X'), 7,
                             cv::Size(1536, 576));
  ...
  for (size_t i = 0; i < detections.size(); ++i) {
    ...
    for (const auto& c : curr) {
      ...
      dets.emplace_back(rect.x + (rect.width >> 1), rect.y + (rect.height >> 1), rect.width,
                        rect.height);
